...
const teeSet = computed(
  () =>
    courseManifest.value?.courses?.[0]?.tee_sets?.[DEFAULT_TEE_SET_ID] ?? {
      holes: {},
    }
);
...
const players = computed(() => eventJson.value?.players ?? []);
      ...
      <template v-else>
        ...
        <div class="event-selector">
          <select v-model="selectedMonthKey">
            <option v-for="m in monthOptions" :key="m.key" :value="m.key">
              {{ m.key }}
            </option>
          </select>
